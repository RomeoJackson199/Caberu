...
BEGIN
  ...
  WHERE id = cancel_appointment.appointment_id
    AND patient_id IN (
      SELECT id FROM public.profiles WHERE profiles.user_id = cancel_appointment.user_id
    );
  ...
  -- Audit log (must use allowed action values)
