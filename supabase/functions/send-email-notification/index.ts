...
// CORS headers - always include all properties
const corsHeaders = {
  ...
  try {
    ...
    if (!isSystem) {
      try {
        ...
        const limitErrorMessage = limitError instanceof Error ? limitError.message : '';
        if (limitErrorMessage.includes('Email limit exceeded')) {
          throw limitError;
        }
