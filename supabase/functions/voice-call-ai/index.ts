  ...
  try {
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case 'send_profile_completion_link': { if(!aPhone) return new Response(JSON.stringify({error:'phone required'}),{status:400,headers:{...corsHeaders,'Content-Type':'application/json'}}); let welcomeFirstName='there'; let welcomeBizName=''; if(bid){const{data:biz}=await supabase.from('businesses').select('name').eq('id',bid).maybeSingle();welcomeBizName=biz?.name||'';} const{data:pt}=await supabase.from('secure_profiles_view').select('first_name').eq('phone',aPhone).maybeSingle();if(pt?.first_name)welcomeFirstName=pt.first_name; const waResult=await sendWhatsAppTemplate({phone:aPhone,contentSid:WHATSAPP_TEMPLATES.PATIENT_WELCOME,contentVariables:{"1":welcomeFirstName,"2":welcomeBizName,"3":"caberu.be/login"},businessId:bid||'',templateName:'patient_welcome'}); return new Response(JSON.stringify({ok:waResult.success,sid:waResult.sid,error:waResult.error}),{headers:{...corsHeaders,'Content-Type':'application/json'}}); }
