  ...
  try {
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case 'send_profile_completion_link': { if(!aPhone) return new Response(JSON.stringify({error:'phone required'}),{status:400,headers:{...corsHeaders,'Content-Type':'application/json'}}); /* Send WhatsApp welcome instead of SMS */ const waResult=await sendWhatsAppTemplate({phone:aPhone,contentSid:WHATSAPP_TEMPLATES.PATIENT_WELCOME,contentVariables:{"1":"there"},businessId:bid||'',templateName:'patient_welcome'}); return new Response(JSON.stringify({ok:waResult.success,sid:waResult.sid,error:waResult.error}),{headers:{...corsHeaders,'Content-Type':'application/json'}}); }
