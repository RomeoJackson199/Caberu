...
export function BusinessProfileDefault({
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Services Section */}
      {showServices && (
        <section id="services" className="py-20 md:py-24 px-4 lg:px-8">
          <div className="container mx-auto max-w-6xl">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {services.map((service) => (
                <Card
                  key={service.id}
                  className="group relative overflow-hidden border hover:border-primary/30 hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                >
                  <CardContent className="p-6">
                    ...
                    {service.duration_minutes && (
                      <div className="mt-4 pt-4 border-t flex items-center gap-1.5 text-sm text-muted-foreground">
      ...
      {/* Team Section */}
      {team.length > 0 && (
        <section id="equipe" className="py-20 md:py-24 px-4 lg:px-8 bg-muted/30">
          <div className="container mx-auto max-w-5xl">
            <div className="text-center mb-14">
              <p className="text-sm font-semibold text-primary uppercase tracking-wider mb-3">Notre {"\u00e9"}quipe</p>
              <h2 className="text-3xl font-bold sm:text-4xl">
                Des professionnels d{"\u00e9"}vou{"\u00e9"}s
              </h2>
              <p className="mt-4 text-muted-foreground text-lg max-w-2xl mx-auto">
                Une {"\u00e9"}quipe exp{"\u00e9"}riment{"\u00e9"}e {"\u00e0"} votre service
              </p>
            </div>
            <div className={cn(
              "grid gap-8",
              ...
              {team.map((member) => (
                <div key={member.id} className="text-center group">
                  <div className="relative mx-auto mb-5">
                    {member.avatar_url ? (
                      <img
                        ...
                        className="mx-auto h-32 w-32 rounded-2xl object-cover shadow-lg group-hover:shadow-xl transition-shadow"
                      />
                    ) : (
                      <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-2xl bg-primary/10 shadow-lg">
                  ...
                  <p className="text-sm text-muted-foreground mt-1 capitalize">
                    {member.role === "dentist" ? "Dentiste" : member.role === "admin" ? "Responsable" : member.role}
                  </p>
                </div>
              ))}
            </div>
      ...
      {hasContactInfo && (
        <section id="contact" className="py-20 md:py-24 px-4 lg:px-8 border-t">
          <div className="container mx-auto max-w-4xl">
            ...
            <div className={cn(
              "grid gap-5",
              // Responsive grid based on number of contact items
              "sm:grid-cols-2",
